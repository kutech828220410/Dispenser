...
namespace 智慧調劑台管理系統_WebApi
{
    ...
    public class tradingController : ControllerBase
    {
        ...
        public string Get(string action, string? code, string? name, string? SN, string? Operator, string? patient_name, string? patient_code, DateTime? operating_time_start, DateTime? operating_time_end, DateTime? prescription_time_start, DateTime? prescription_time_end)
        {
            ...
            if (SN != null)
            {
                list_value_buf = list_value_buf.Where(a => a[(int)enum_trading_page.藥袋序號].ObjectToString() == SN).ToList();
            }
            if (Operator != null)
            {
            ...
            {
                list_value_buf = list_value_buf.Where(a => a[(int)enum_trading_page.病人姓名].ObjectToString() == patient_name).ToList();
            }
            if (patient_code != null)
            {
                list_value_buf = list_value_buf.Where(a => a[(int)enum_trading_page.病歷號].ObjectToString() == patient_code).ToList();
            }
            if (name != null)
            {
            ...
            for (int i = 0; i < list_value_buf.Count; i++)
            {
                ...
                class_Trading_Data.GUID = list_value_buf[i][(int)enum_trading_page.GUID].ObjectToString();
                ...
                class_Trading_Data.開方時間 = list_value_buf[i][(int)enum_trading_page.開方時間].ToDateTimeString();
                class_Trading_Data.備註 = list_value_buf[i][(int)enum_trading_page.備註].ObjectToString();
                class_Trading_Data.手術房號 = "A01";
